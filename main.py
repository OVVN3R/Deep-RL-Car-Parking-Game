...
# ---SPRITES/OBJECTS (PLAYGROUND VERSION)---
car = Car()
spot = ParkingSpot()
obstacle = Obstacle()
moving_obstacle = MovingObstacle(vertical=True) 
env = ParkingEnv(render=True)
agent = CarAgent(env)
...
while running:
        ...
        if event.type == pg.KEYDOWN: 
            if game_state == "interface": # interface event handling
                ...
                if event.key == pg.K_RETURN: # handle level screen selection after exiting brute navigation
                    ...
                    if procedures.peek(screen_stack) == "ai_level_screen" and (current_index in range(3)): 
                        # handle ai level screen selection
                ...
                if event.key == pg.K_RETURN: # return to the main menu
                    ...
                    if procedures.peek(screen_stack) == "singleplayer_level_screen": 
                        # return to the singleplayer level screen
                        screens_file.singleplayer_level_screen()
                        procedures.highlight_selection()

                    elif procedures.peek(screen_stack) == "ai_level_screen": 
                        # return to the ai level screen
                        screens_file.ai_level_screen()
                        procedures.highlight_selection()
                    
                    elif procedures.peek(screen_stack) == "speedrun_level_screen": 
                        # return to the speedrun level screen
    ...
    if game_state == "interface": 
        ...
        if enter_pressed: # handle level screen selection and music 
            ...
            if procedures.peek(screen_stack) == "settings_screen":
    ...
    # Game Over -Prototype 2-
        ...
        for i, line in enumerate(lines): # read the lines of the file
            ...
            if int(level) == current_level: # check if the level is the same as the current level
                ...
                if score == "NULL" or elapsed_time < float(score): 
                    # check if the score is NULL or the elapsed time is less than the score
